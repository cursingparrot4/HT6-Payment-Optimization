...
8. All people, cards, transactions, and values are synthetic. No banking credentials, real PII, or real-money movement enters the system.
...
- All data is **synthetic**. No real money moves, no live banking/card credentials, no real PII.
...
/data          # synthetic card catalog + generated training data
...
- **Privacy/security:** all data synthetic; no credentials collected; no real PII; state kept locally / in-memory or a local SQLite seeded with synthetic data. Say this explicitly in the writeup.
...
- `GET /demo-scenario` -> Sarah's synthetic cards, purchases, reference date, and manual intent presets
...
1. Load a synthetic card portfolio (pre-seeded; allow light editing).
...
> **Sarah** has three fictional cards (Aurora Bonus, Summit Journey, and Harbor Rent). She types: *"I'm applying for a mortgage in 3 months so I need to keep my credit utilization low, but I'd still like to hit my Aurora signup bonus, and I pay $2,200 rent."*
>
...
---
## 12. Synthetic data (in `/data/`)
- A catalog of ~6–10 synthetic cards with realistic-but-fake names, varied reward structures (a rent-friendly card, a travel card, a flat-cashback card, a big-signup-bonus card, a low-limit card to force interesting constraints), varied statement/due days.
- A default portfolio of 3–4 cards for the demo persona.
- A month of ~15–25 synthetic purchases including rent, groceries, dining, travel, and a couple of large one-offs.
- Attribute any public reward-structure inspiration; keep all numbers fake.
---
...
- ❌ No real money, real credentials, or real PII. Synthetic only.
